...
class RSAKeyPair:
    ...
    e: int      # Public exponent (Khóa công khai)
    d: int      # Private exponent (Khóa bí mật)
    n: int      # Modulus = p * q (Khóa công khai)
...
class SharedSecretRSADemo:
    ...
    (Lưu ý: RSA không phải là thuật toán sinh khóa chung tự nhiên như DH, 
    nhưng có thể dùng để 2 bên mã hóa gửi số bí mật riêng cho nhau để thống nhất 1 số chung).
    """
    def __init__(self, root):
        ...
        # Khóa RSA của hai bên (Mỗi bên tự có 1 cặp Private - Public)
    ...
    def _build_ui(self):
        ...
        # Khu vực chính: Bên A và Bên B (Chia đôi màn hình)
        ...
        # Khung hiển thị mật khẩu chung (2 hộp riêng biệt để chứng minh 2 bên tính ra giống nhau)
    ...
    def _build_key_frame(self, frame):
        ...
        # Chỉ in ra Public key vì Private Key trong thực tế không được chia sẻ
    ...
    def _build_side(self, frame, side: str):
        ...
        # Sinh một số ngẫu nhiên ban đầu để demo cho lẹ
        self.secret_entry.insert(0, str(secrets.randbelow(999999)))
        
        # Nút bấm kích hoạt việc A gửi cho B hoặc B gửi cho A
        ...
        # Phần hiển thị dữ liệu nhận được từ đối phương (dạng Encrypted và Decrypted)
        ...
        # Lưu tham chiếu widget Text box vào biến phù hợp
        if side == "A":
    ...
    def generate_keys(self):
        ...
        # Cập nhật hiển thị (Bên A và B công khai (e,n) cho nhau)
    ...
    def send_secret(self, from_side: str):
        ...
        Quy tắc RSA: Mã hóa bằng Public Key của người nhận, thì chỉ Private Key của người đó mới mở được.
        """
        try:
        ...
        if from_side == "A":
            ...
            # A gửi cho B => A dùng Public Key của B (e, n của B) để mã hóa
            encrypted = pow(secret, self.key_B.e, self.key_B.n)
            
            # Quá trình nhận: B dùng Private Key của chính mình (d, n của B) để giải mã
            decrypted = pow(encrypted, self.key_B.d, self.key_B.n)   # B giải mã
            
            # Ghi log vào màn hình của B
    ...
    def create_shared_secret(self):
        ...
        Bước này mô phỏng cả 2 bên tự tính toán độc lập ra một "Shared Secret" chung
        để dùng cho mã hóa đối xứng (VD: AES) sau này.
        """
        if not self.key_A or not self.key_B:
        ...
        # Số bí mật của A * Số bí mật của B mod (n của A * n của B)
        self.shared_secret = (self.secret_received_A * self.secret_received_B) % (self.key_A.n * self.key_B.n)
        
        # Hiển thị riêng cho từng bên (Mỗi bên tự tính bằng số của mình gửi đi kết hợp số nhận về)
    ...
    def build_key_pair(self, p: int, q: int, e: int) -> RSAKeyPair:
        ...
        n = p * q
        phi = (p - 1) * (q - 1)
        # Sử dụng pow có sẵn của Python để tính nghịch đảo modulo nhanh (chỉ hỗ trợ Python 3.8+)
