...
class RSAKeyPair:
    ...
    e: int          # Số mũ công khai (public exponent)
    d: int          # Số mũ riêng tư (private exponent)
    n: int          # n = p * q (modulus)
...
class SignatureData:
    ...
    algorithm_name: str     # Tên thực tế dùng trong hashlib
    message: str            # Thông điệp đã được mã hóa XOR (nếu có)
    raw_hash_int: int       # Giá trị hash nguyên bản dưới dạng số nguyên
    reduced_hash: int       # Hash đã lấy modulo n
    signature_int: int      # Chữ ký số (đã ký bằng private key)
...
class VerificationData:
    ...
    raw_hash_int: int
    reduced_hash: int
    decrypted_signature: int
    is_valid: bool          # True nếu chữ ký hợp lệ
...
def xor_cipher_base64(text: str, password: str, encrypt: bool = True) -> str:
    ...
    if encrypt:
        ...
        # Bọc Base64 để chuỗi an toàn khi hiển thị trong Text widget
...
class DigitalSignatureDemoApp:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.e_var = tk.StringVar(value="65537")
    ...
    def forward_packet(self) -> None:
        ...
        self.receiver_public_key_var.set(self.hacker_public_key_var.get())   # Chuyển khóa công khai
